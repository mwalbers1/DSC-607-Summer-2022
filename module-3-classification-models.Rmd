...
```{r}
medals_decision_tree$gold <- ifelse(medal_type == "Gold Medal", 1, 0)
medals_decision_tree$silver <- ifelse(medal_type == "Silver Medal", 1, 0)
medals_decision_tree <- ifelse(medal_type == "Bronze Medal", 1, 0)
```
...
```{r}
medals_knn_train <- shuffled_medals_knn[1:split,]
medals_knn_test <- shuffled_medals_knn[(split+1):nrow(shuffled_medals_knn),]
```
...
```{r}
mean(p == actual_medals.test)
```

### Calculate misclassification rate of kNN model

```{r}
num.incorrect <- sum(p != actual_medals.test)
num.correct <- sum(actual_medals.test)
