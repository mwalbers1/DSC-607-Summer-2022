...
Find frequent itemsets for both Nyc and Ny-Other baskets containing risk factors
...
```{r}
nyc_support.df <- DATAFRAME(nyc_support)
```
...
```{r}
nyc_basket.plot <- nyc_basket[!nyc_basket %in% "New"]
```


```{r fig.height=7}
itemFrequencyPlot(nyc_basket.plot,
...
```{r}
nyc_basket.plot2 <- nyc_basket[!nyc_basket %in% c("New", "Kings,Hispanic", "Queens,Hispanic", "Suffolk,Hispanic", "Bronx,Hispanic", "Nassau,Hispanic", "Kings,African.American", "Bronx,African.American", "Queens,African.American")]
```
...
```{r}
ny_other_basket.plot <- ny_other_basket[!ny_other_basket %in% "St."]
```


```{r fig.height=7}
itemFrequencyPlot(ny_other_basket.plot,
...
```{r}
ny_other_basket.plot2 <- ny_other_basket[!ny_other_basket %in% c("St.","Westchester,Hispanic","Erie,Hispanic","Monroe,Hispanic",
